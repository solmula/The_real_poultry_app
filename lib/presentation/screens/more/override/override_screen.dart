...
class OverrideScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer2<LiveDataProvider, CommandProvider>(
        builder: (context, live, cmd, _) {
          final d = live.data;
          final isViewer = context.read<AuthProvider>().isViewer;

          final commandDisabled = isViewer || cmd.isPending;
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 8,
            children: options.map((opt) {
              final isActive = currentLabel.toUpperCase() == opt.toUpperCase();
      ...
      child: OutlinedButton.icon(
        onPressed: disabled
            ...
            : () async {
                ...
                try {
                  await cmd.clearCommands();
                  if (!context.mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                          'All overrides cleared — ESP32 resuming automatic control'),
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                          'Failed to clear overrides — no internet connection'),
                      backgroundColor: AppColors.statusCritical,
